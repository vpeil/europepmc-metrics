...
option initial_file => (
    is => 'ro',
    format => 's',
    default => sub {'data/pmid.csv'},
    doc => "Specify the initial input file, default is 'data/pmid.json' in the pwd.",
    );
option epmc_file => (
    is => 'ro',
    format => 's',
    default => sub {'data/epmc.json'},
    doc => "Specify the europe pmc file, default is 'data/epmc.json' in the pwd.",
    );
option source => (
    is => 'ro',
    short => 's',
...
sub _fetch_source {
    ...
    my $exporter = Catmandu->exporter('default', file => "data/$source.json");
